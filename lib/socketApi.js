...
const
  bucket={
    market: new TokenBucket({ bucketSize: 100, tokensPerInterval: 100, interval: "second" })
  };
...
class SocketClient {
  ...
  _createSocket(url) {
    ...
    this._ws.onclose = () => {
      ...
      setTimeout(() => this._createSocket(this._ws._url), 1000);
    };

    this._ws.onerror = err => {
      console.log('ws error', this.name, JSON.stringify(err));
      setTimeout(() => this._createSocket(this._ws._url), 1000);
    };
        ...
        switch(message.method) {
          ...
          case "public/heartbeat":
            if (this._ws !== null && this._ws.readyState === WebSocket.OPEN) {
              const pong={ id: message.id, method: "public/respond-heartbeat" };
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          if (this._promises.has(requestId)) {
            ...
            reject({"code":"408","error":"Request Timeout","data":msg});
          }
        }, 10000);
      });
...
var CryptoSocket = function(url, keys) {
  ...
  this._bucket={
    user: new TokenBucket({ bucketSize: 150, tokensPerInterval: 150, interval: "second" })
  };
...
CryptoSocket.prototype.login = async function(handler, api) {
  ...
  switch(handler.socket._ws.readyState) {
    case 1: handler.socket._ws.send(JSON.stringify(options)); break;
    case 0: setTimeout(handler.login, handler.timeout, handler, api); break;
    default: handler.socket._ws.terminate(); break;
  };
...
CryptoSocket.prototype.subscribeOrderbook = async function(symbol,depth,options={}) { // async
  ...
  const remaining=await bucket["market"].removeTokens(1);
...
CryptoSocket.prototype.subscribeOrders = async function(symbol) { // async
  ...
  const remaining=await this._bucket["user"].removeTokens(1);
  const output=await this.socket.request('subscribe',channel,undefined);
  return output;
};
