...
CryptoPublic.prototype.getOrderBook = async function(symbols,options) {
  var path="/public/get-book",sep="?";
  Object.keys(options).forEach(key => { path+=sep+"="+options[key]; sep="&"; });
  return await this.query(path);
};

CryptoPublic.prototype.getCandles = async function(symbols,options) {
  var path="/public/get-candlestick",sep="?";
  Object.keys(options).forEach(key => { path+=sep+"="+options[key]; sep="&"; });
  return await this.query(path);
};

CryptoPublic.prototype.getTrades = async function(symbols,options) {
